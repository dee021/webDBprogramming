...
// templateList와 templateHTML를 객체명 template으로 정리
// 기존 함수 호출 방식에서 객체 메소드 호출 방식으로 변경
var template = {
    ...
    if(pathname == '/') {
        if(queryData.id === undefined){
            ...
            fs.readFile(`data/${queryData.id}`, 'utf-8', function(err, description) {
                ...
                var list = template.list(filelist);
                var templ = template.HTML(title, list, `<h2>${title}</h2><p>${description}</p>`,
                `<a href="/create">create</a>
