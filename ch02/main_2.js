    ...
    if(pathname == '/') {
        if(queryData.id === undefined){ // db에서 조회
            ...
            db.query(`SELECT * FROM topic`, function(error, topics) {
                ...
                // db.query(`SELECT * FROM topic WHERE id=${queryData.id}`, function(error2, topic) { // topic은 list 형태로 읽어옴
                    /* 
                    sql문에 id 값을 직접 넣으면 해킹의 위험이 있으므로 수정
                    아래와 같이 바꾸면 ? 자리에 id가 치환되며, 공격 의도가 있는 코드를 알아서 걸러줌
                    */
